...
const CertCard = ({
  ...
  return <motion.div ref={ref} initial={{
    ...
    duration: 0.5,
    delay: index * 0.1
  }} className={`glass-effect rounded-xl p-6 hover-lift ${cert.featured ? 'md:col-span-2 lg:col-span-1 border-accent/50' : ''}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        {cert.featured && <span className="text-xs font-medium text-accent bg-accent/10 px-2 py-1 rounded-full animate-pulse-glow">
            Featured
          </span>}
      </div>
      
      <h3 className="font-semibold text-lg mb-1">{cert.title}</h3>
      {cert.code && <span className="text-xs text-accent font-medium">({cert.code})</span>}
      <p className="text-sm text-muted-foreground mt-2 mb-4">{cert.issuer}</p>
      
      <Button asChild variant="ghost" size="sm" className="text-accent hover:text-accent hover:bg-accent/10 -ml-3">
...
const Certifications = () => {
  ...
  return <section id="certifications" className="py-20 md:py-32 gradient-bg">
      <div className="container mx-auto px-4">
        ...
        {featuredCert && <motion.div initial={{
        opacity: 0,
        scale: 0.95
      }} animate={isInView ? {
        opacity: 1,
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {otherCerts.map((cert, index) => <CertCard key={cert.id} cert={cert} index={index} />)}
        </div>
      </div>
    </section>;
};
